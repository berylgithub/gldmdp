 ...
 * @version 1.0
 */
public class PercentageQue extends TLController
{	public static final String shortXMLName="Percentage-Queue";
	int numCycle;
	/**
	 * The constructor for TL controllers
	 * @param The model being used.
	 */
	public PercentageQue(Infrastructure infras) {
	    super(infras);
...
//					tld[i][j].setGain(0);
                            ...
                            //tld[i][j].setGain(1);
                            double percentage=(double)tld[i][j].getTL().getLane().getNumBlocksWaiting()/tld[i][j].getTL().getLane().getLength();
                            if(percentage<0.3){
                                tld[i][j].setGain(1);
                            }
                            ...
                            System.out.println(tld[i][j].getTL().getId()+" "+tld[i][j].getTL().getLane().getNumBlocksTaken());
                    }
                    System.out.println("");
		}
		return tld;
