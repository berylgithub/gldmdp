 ...
 * @version 1.0
 */
public class MDP_5segment_5step_specialnodes_type2 extends TLController {

    public static final String shortXMLName = "MDP_5segment_5step_specialnodes_type2";
...
//    ArrayList<ActionSeqContainer> arrASC=new ArrayList<>();
     ...
     */
    public MDP_5segment_5step_specialnodes_type2(Infrastructure infras) throws IOException {
        super(infras);
        ...
        for (int i = 0; i < arrMultiSBAC.length; i++) {
            if (i == 6 || i == 7) {
                arrMultiSBAC[i] = loadActionFile("Value Iteration Result_5-segment_5-step_random_specialnode_type2-" + i + "_train2_0,5gamma.txt");
            }
...
//        arrSBAC = loadActionFile("Value Iteration Result_3-segment_5-step_random_neighboorStates.txt");
        ...
        for (int i = 0; i < arrMultiSBAC.length; i++) {
            for (int j = 0; j < arrMultiSBAC[i].size(); j++) {
                System.out.println(arrMultiSBAC[i].get(j).getState() + " " + arrMultiSBAC[i].get(j).getAction());
            }
            System.out.println("");
        }
        ...
        for (int i = 0; i < tld.length; i++) {
            ...
            for (int j = 0; j < tld[i].length; j++) {
                try {
                    ...
                    Logger.getLogger(MDP_5segment_5step_specialnodes_type2.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
        for (int i = 0; i < tld.length; i++) {
...
//            System.out.println(manualPolicyApplier(tempState)+" "+tempState);
                ...
                for (int k = 0; k < arrMultiSBAC[i].size(); k++) {
                    ...
                    if (tempState.equals(arrMultiSBAC[i].get(k).getState())) {
                        ...
                        System.out.println("Node " + i + " " + arrMultiSBAC[i].get(k).getState() + " " + laneNumber + " " + currentCycle);
                    }
                }
...
//            }
    ...
    public String getXMLName() {
        return "MDP_5segment_5step_specialnodes";
    }
}
