...
{	
	...
	*/	
	...
	protected void recalcP(Sign tl, int pos, Node destination, boolean light, Sign tl_new, int pos_new, int Ktl)
	{
		...
		Enumeration enumm = count[tlId][pos][desId].elements();
		while(enumm.hasMoreElements()) {
			CountEntry current = (CountEntry) enumm.nextElement();
		...
		enumm = count[tlId][pos][desId].elements();
		while(enumm.hasMoreElements()) {
			source += ((CountEntry) enumm.nextElement()).sameSourceWithKtl(currentsituation);
		}
